...
def create_dataset_profile(csv_path: str) -> dict:
    ...
    categorical_columns = [
        c for c, meta in columns.items() if meta.get("semantic_type") in ("categorical", "text", "identifier", "boolean")
    ]
    date_columns = [c for c, meta in columns.items() if meta.get("semantic_type") == "date"]

    if columns:
        overall_completeness = sum(100 - float(meta.get("null_pct", 0.0)) for meta in columns.values()) / len(columns)
    else:
